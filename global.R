...
#this will be used in sideGraphUi: it is a common ui for both x and y side graphs
        ...
        if(tolower(side) == "x"){
          ...
          if( req(input$sideVariable) == "default"){
            ...
            if(req(input$sideGraphType) == "density"){
              ...
              graph1 <- geom_ysidebar(aes_string(x = input$sideVariable, fill= color, linetype = linetype),
                                      stat = req(input$barStat), width = req(input$barWidth), #req(input$stat)
                                      position = req(input$barPosition), orientation = req(input$barOrientation))
            }else if(req(input$sideGraphType) == "box plot"){
...
#module for aesthetic:-----------
"arguments:
id = character. ID for the module
df = data frame.
plotType = character. type of graph choosen. It is require for density to adjust the transparency.

return:
"
module_ui_aesthetic <- function(id, df = NULL, plotType = "density"){
  ns <- NS(id)
  div(
    ...
    #ui for color: color option need to be provided from the server logic
    selectInput(inputId = ns("colorSet"), label = "Add color", choices = list("none")),
    
    #option to provide color
    #provide option to auto fill the color or customize it
    conditionalPanel(condition = "input.colorSet !== 'none'", ns = ns,
                     radioButtons(inputId = ns("autoCustome"), label = NULL, choices = c("auto filled","customize"), selected = "auto filled")
    ),
    #ui for entering color
    conditionalPanel(condition = "input.colorSet !== 'none' && input.autoCustome == 'customize'", ns = ns,
                     fluidRow(
                       style = "padding: 0",
                       column(9, textAreaInput(inputId = ns("colorAdd"), label = "Enter colors",
                                               placeholder = "comma or space separated. \nE.g. red, #cc0000, BLUE", width='100%'),
                              bsTooltip( id = ns("colorAdd"), title = "comma or space separated. \nE.g. red, #cc0000, BLUE",placement = "bottom")
                       ),
                       column(3, colourpicker::colourInput(inputId = ns("customePick"), label = "click", showColour = "background",value = "red"),
                              bsTooltip(id= ns("customePick"), title = "Choose color", placement = 'bottom'))
                     )
                     
    ),
    
    #UI for positioning of density and alpha
    conditionalPanel(condition = "plotType === 'density' && input.colorSet !== 'none'", ns=ns,
                     fluidRow(
                       column(6, {
                         positions<- c("stack", "identity","fill")
                         selectInput(inputId = ns("densityPosition"), label = "Position", choices = c("default", sort(positions)), selected = "default")
                       }),
                       column(6, sliderInput(inputId = ns("alpha"), label = "Transparency", min = 0.01, max = 1, value = 1))
                     )),
    
    #ui for adding shape and linetype
    conditionalPanel(condition = "plotType !== ''", ns = ns,
                     {
                       ...
                       checkboxGroupInput(inputId = ns("shapeLine"), label = "Add more aesthetic", choiceNames = shapeLineChoice, choiceValues = c("Shape", "Line type"), inline = TRUE)
                     }
    ),
    
    #ui for shape
    uiOutput(outputId = ns("UiShapeSet")),
    #ui for line
    uiOutput(outputId = ns("UiLineSet"))
  )#end of div aesthetic
}

#module server for aesthetic: include updating the ui
"argument:
id
df = data frame
plotType = character.
stat = character. statistic method. Base on the method aesthetic will be updated.
xVar, yVar = character. Variable/column name of x- and y-axis respectively. It is require to show only relevant variables to provide as option.
xVarType = character. Type of columns, either numeric or character. It is require for histogram only.
histMean = character. Specify whether to determine group mean or not.
histGroupVariable = character. Variable name to group by for determining mean. If histMean = 'group mean', than specify base on the given variables.
lineConnectPath = character. Variable name to be used for grouping line. It is necessary for line graph.

"
module_ser_aesthetic <- function(id, df = NULL, plotType = "none", stat = c("t.test", "wilcoxon.test", "kruskal-wallis", "anova"),
                                 xVar = NULL, yVar = NULL,
                                 xVarType = c("numeric", "character"),
                                 histMean = "group mean",
                                 histGroupVariable = NULL,
                                 lineConnectPath = NULL
                                 ){
  moduleServer(id, function(input, output, session){
    
    req(is.data.frame(df), plotType != "none")
    ptable <- reactive(df)
    ...
    varColorChoice <- reactive({
      ...
      if(plotType == "histogram" && xVarType %in% c("integer", "numeric", "double")){# && input$histMean == "group mean"){
        #Histogram: if variable for group mean is chosen, then provide only that variable for color, no other variables!
        if(histMean == "group mean"){
          histGroupVariable
        }
      }else{
        #generic
        if(plotType %in% c("box plot","violin plot", "line", "scatter plot", "bar plot")){
    ...
    updateSelectInput(inputId = "colorSet", label= "Add color", choices = c("none",xVar), selected = "none")
    #reset the customize option
    ...
    #option for shape and line
    if(plotType == "none"){
      #show the default options: useful if user wants to check the available feature
      output$UiShapeSet <- renderUI({
        if( plotType == "none" && ( req(input$shapeLine) == "Shape" || all( c("Shape", "Line type") %in%  input$shapeLine) ) ) selectInput(inputId = "shapeSet", label = "Variable for shape", choices = "none")
      })
      output$UiLineSet <- renderUI({
        if( plotType == "none" && ( req(input$shapeLine) == "Line type" || all( c("Shape", "Line type") %in%  input$shapeLine) ) ) selectInput(inputId = "lineSet", label = "Variable for line type", choices = "none")
      })
    }else{
      ...
      allVar <- colnames(ptable())
      choiceVar <- allVar[allVar != yVar]
      
      output$UiShapeSet <- renderUI({
        if(plotType != 'bar plot' && req(input$shapeLine) == "Shape") selectInput(inputId = "shapeSet", label = "Variable for shape", choices = choiceVar, selected = var)
      })
      output$UiLineSet <- renderUI({
        if( req(input$shapeLine) == "Line type" && (plotType == "line" && lineConnectPath != "none")){
          selectInput(inputId = "lineSet", label = "Variable for line type", choices = lineConnectPath, selected = lineConnectPath)
        }else if( req(input$shapeLine) == "Line type" && plotType != "line"){
          selectInput(inputId = "lineSet", label = "Variable for line type", choices = choiceVar, selected = var)
        }
      }) 
    }#end of shape and line
    
  })
}
#module for providing option of x- and y-axis------------------
"arguments:
id = character.
plotType = character. Type of graph chosen.
axis = character. Option for x- and y-axis. 'both' will provide both the x- and y-axis, else x-axis will be provided.
      It depends on the type of graph. e.g. box plot require both x- and y-axis, but density require only x-axis.


Usage:
This module will be in the server logic. Use it for updating the UI of x- and y-axis.
return:
It will return ui objects of x-axis and/or y-axis.
"
module_ser_xyAxis <- function(id, df = NULL, plotType = "none", axis = c("both", "x")){
  
  moduleServer(id, function(session, input, output){
    req(is.data.frame(df), plotType != "none")
    #convert the table to reactive
    ptable <- reactiveVal(df)
    #get the default column for x-axis: return 1 column name
    defaultXVar <- selectedVar(data = ptable(), check = "character")
    #option for x axis
    output$uiXAxis <- renderUI({
      tryCatch({
        #require checks to prevent crash: `...1`
        validate(
        ...
        selectInput(inputId = "xAxis", label = "X-axis", choices = colnames(ptable()), selected = defaultXVar)
    ...
    })#end of option for x-axis
    
    
    #for y-axis, the 'axis' must be both
    if(axis == "both"){
      #get the default variable of y-ais: it must be numeric
      variableYAxis <- selectedVar2(data = ptable(), check = "numeric", index = 2)
      #provide option for y-axis
      output$uiYAxis <- renderUI({
        tryCatch({
          ...
          selectInput(inputId = "yAxis", label = "Y-axis", choices = colnames(ptable()), selected = variableYAxis)
        }, error = function(e){
          msg <- e
          print(msg$message)
        })
...
#module for box plot---------------------------
#ui logic for box plot
module_box_plot <- function(id, plotType = c("none", "box plot")){
  ns <- NS(id)
  #ui for x axis
  uiOutput("uiXAxis")
  #ui for y axis
  uiOutput("uiYAxis")
  #ui for box width
  sliderInput(inputId = ns("boxWidth"), label = "box width", min = 0.001, max = 2, value = 0.1)
  #ui for aesthetic: color, shape and line options need to be provided from server logic
  module_ui_aesthetic(id = ns("boxAesthetic"))
  "
  note:
  shape and line setting have to be implemented from the server side
  "
}
#server logic of box plot
module_ser_box_plot <- function(id, df = NULL, plotType = c("none", "box plot")){
  moduleServer(id, function(session, input, output){
    req(is.data.frame(df))
    ptable <- reactiveVal(df)
    #create ui for option of x- and y-axis
    module_ser_xyAxis(id = id, df = ptable(), plotType = plotType, axis = "both")
    module_ser_aesthetic(id = "boxAesthetic")
    #update the shape and line
    
  })
}
#end module for box plot---------------------------
...
moduleLineSecSer <- function(id, df = NULL, pltType = NULL, yPr= NULL,
                             ySec = NULzL, yCol = NULL, titlePr = NULL,
...
#selection for x-axis and first variable
