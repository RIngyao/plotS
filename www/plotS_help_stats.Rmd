...
<p style="font-size:20px;"> 
<font size = "4">
It is a non-parametric tests for comparing two groups. If the data has more than two groups to compare than it will prompt a warning message to continue or stop the computation. Just as in t-test, proceeding the computation will by default analyse all possible combinations of two variables per group. Users can later add reference group or add variables to compare. Similar steps as in **t-test** for adding variables to compare.
