                                            ...
                                            dropdownButton(inputId = "sideDropdownButton", right = TRUE, width="550px", label = tags$b("Side graph", style="color:#C622FA"), circle = FALSE, size = "default", tooltip = tooltipOptions(title = "Add or remove side graph", placement = "bottom"), icon = icon("sliders"),
                                                           jqui_draggable(
                                                             div(
                                                               ...
                                                               style = "text-align:center; overflow-y:auto; max-height: 450px; box-shadow:0px 2px 20px -15px inset;",
                                                               h4("Add graph on the x- and y-sides of the main graph.", align = "center", style = "color:green; margin-bottom:5px"),
                                                               tags$p("** Does not support when two-way ANOVA or secondary y-axis is active", style = "text-align:center; margin-bottom:20px;"),
...
server <- function(input, output, session){
  ...
  #update label for primary y-axis label
  observe({
  ...
  observe({
    ...
    #get y label
    if(isTruthy(input$secYLable)){
      secTitle <- reactive(input$secYLable)
    }else{
 ...
 filterMsg <- reactiveVal(NULL) #msg for filter
  ...
  #update alternativeHypothesis
  observe({
    req(input$stat, input$plotType)
    updateSelectInput(inputId = "alternativeHypothesis", label = "Alternative hypothesis", choices = hypothesis)
    # addTooltip(session, id = "alternativeHypothesis", title = "Must not choose alternative hypothesis for the sole purpose of attaining significance", placement = "top")
  })
  ...
  #
    ...
    linC <- reactive({if(figType() == "line") {req(input$lineConnectPath) }})
    ...
    geomType <- reactive({
      switch(figType(),
             ...
             "line" = if(connectVar() == 1){
               ...
               #   geom_line(linewidth = freqPolySize(), group = connectVar())
               # }
             }else{
        ...
        validate(
          need(
            finalPltError() == 0, if( is.null(finalPltErrorMsg()$message) || finalPltErrorMsg()$message == "" || str_detect(finalPltErrorMsg()$message, "sData1")){
              ""
            }else{ finalPltErrorMsg()$parent$message }
          )
        )
  ...
  observe({
    # browser()
    #don't show for two-way anova
    if(req(input$stat) == "anova") req(input$pairedData != "two")
